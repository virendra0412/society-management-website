        ...
        <div>
          ...
          <div className="inline-flex items-center gap-2 bg-teal/20 border border-teal/30 rounded-full px-3.5 py-1.5 mb-6">
            ...
            <span className="text-teal text-xs font-semibold tracking-wide uppercase">
              Free for societies up to 50 units
            </span>
          </div>
          ...
          <h1 className="font-display text-4xl md:text-5xl xl:text-6xl font-bold text-white leading-tight mb-5">
            Your Society,
            <br />
            <span className="text-teal">Managed.</span>
          </h1>

          <p className="text-white/60 text-lg leading-relaxed mb-8 max-w-lg">
            From visitor gate entry to maintenance bills — one app your entire
            society will actually use. No hardware. No complexity. Free to start.
          </p>
          ...
          <div className="flex items-center gap-5 flex-wrap">
            <div className="flex -space-x-2">
              {["R","A","P","S","M"].map((l,i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full bg-teal/30 border-2 border-navy flex items-center justify-center text-teal text-xs font-bold"
                >
                  {l}
                </div>
              ))}
          ...
          <div className="animate-float relative">
            ...
            <div className="w-64 bg-[#0a1628] rounded-[2.5rem] border border-white/10 shadow-2xl shadow-black/60 overflow-hidden p-1.5">
              ...
              <div className="bg-teal mx-1 rounded-xl px-3 py-2.5 mb-2">
                <p className="text-white/80 text-[10px] mb-0.5">Good morning,</p>
                <p className="text-white font-display font-bold text-sm">Secretary Rajesh 👋</p>
              </div>
              ...
              <div className="space-y-1.5 px-1 pb-2">
                {phoneScreenItems.map((item, i) => (
                  <div
                    key={i}
                    className="bg-white/5 rounded-xl px-3 py-2 flex items-start gap-2.5"
                  >
                    ...
                    <div
                      className={`w-2 h-2 rounded-full mt-1 flex-shrink-0 ${
                        ...
                        item.dot === "amber" ? "bg-amber" : "bg-teal"
                      }`}
                    />
                  </div>
                ))}
